...
// Composes the idle and running shadow sheets.
...
// Shadow generates the idle and running shadow sheets in gfxDir.
func Shadow(gfxDir string) error {
	...
	for _, d := range directions {
		...
		shadows[d] = resize(makeShadow(src), size, size)
	}

	if err := writeShadowIdle(gfxDir, shadows); err != nil {
		return err
	}
	return writeShadowRunning(gfxDir, shadows)
}
...
func writeShadowIdle(gfxDir string, shadows map[string]*image.NRGBA) error {
	size := shadows[directions[0]].Bounds().Dx()
	sheet := newCanvas(size, size*len(directions))
	for i, d := range directions {
		pasteAt(sheet, shadows[d], 0, size*i)
	}
...
func writeShadowRunning(gfxDir string, shadows map[string]*image.NRGBA) error {
	...
	for ri, d := range directions {
		for fi := range frames {
			pasteAt(sheet, shadows[d], size*fi, size*ri)
		}
	}
