...
func CLI(args []string, stderr io.Writer) int {
	...
	fs.Usage = func() {
		if _, err := fmt.Fprintln(stderr, "usage: build [-gfx <dir>] [-mod <dir>] [-mods <dir>] {running|shadow|sheets|knight|corpse|mining|armed-idle|all|install|uninstall}"); err != nil {
			return
		}
		fs.PrintDefaults()
	}
...
func dispatch(cmd, gfxDir, modDir, modsDir string) error {
	...
	case "mining":
		return Mining(gfxDir)
	case "armed-idle":
		return ArmedIdle(gfxDir)
	case "all":
		return buildAll(gfxDir)
	case "install":
...
func buildAll(gfxDir string) error {
	steps := []func(string) error{Run, Shadow, Sheets, Knight, Corpse, Mining, ArmedIdle}
